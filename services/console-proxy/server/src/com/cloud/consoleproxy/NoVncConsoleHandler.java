...
public class NoVncConsoleHandler implements HttpHandler {
    ...
    public NoVncConsoleHandler(){
        s_logger.warn("NoVNC console hit");
    }
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        try {
            if (s_logger.isTraceEnabled())
                s_logger.trace("AjaxHandler " + httpExchange.getRequestURI());
            ...
            httpExchange.sendResponseHeaders(500, -1);     // server error
        } finally {
            httpExchange.close();
        }
    }
    ...
    private void doHandle(HttpExchange httpExchange) throws IOException {
        ...
        if (event != 0) {
            if (ajaxSessionId != 0 && ajaxSessionId == viewer.getAjaxSessionId()) {
                ...
                sendResponse(httpExchange, "text/html", "OK");
            } else {
                ...
                sendResponse(httpExchange, "text/html", "Invalid ajax client session id");
            }
        } else {
            if (ajaxSessionId != 0 && ajaxSessionId != viewer.getAjaxSessionId()) {
                ...
                handleClientKickoff(httpExchange, viewer);
                ...
                handleClientStart(httpExchange, viewer, title != null ? title : "", guest);
            } else {

                if (s_logger.isTraceEnabled())
                    s_logger.trace("Ajax request indicates client update");

                handleClientUpdate(httpExchange, viewer);
            }
        }
    }
